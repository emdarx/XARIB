...
game_waiting = (function () {
    return function (str) {
        ...
        __init50Hss__();
        var Bearish = getAlgorithmNumberHash(str.md5, 1.02, 2);
        ...
        if (isLowerCoefficient) {
            var randomPercentage = (Math.random() * (0.75 - 0.40) + 0.40);
            var adjustedCashout = (parseFloat(Bearish) * randomPercentage).toFixed(2);
            finalCashout = Math.max(adjustedCashout, 1.01);
...
if (finalCashout < 1.30) {
    finalCashout = 0;
} else if (finalCashout < 1) {
    var randomValue = (Math.random() * (1.20 - 1.01) + 1.01);
    finalCashout = randomValue.toFixed(2);
}

    var difference = Math.abs(parseFloat(Bearish) - parseFloat(Bullish));
        if (difference < 0.1) {
            finalCashout = Math.abs(parseFloat(Bearish) - parseFloat(Bullish)).toFixed(2);
        }
        // Check if the difference is between 0.1 and 0.4
        else if (difference >= 0.1 && difference <= 0.4) {
            var subtractionResult = Math.abs(parseFloat(Bearish) - parseFloat(Bullish));
            var adjustedSubtraction = (subtractionResult * 2 - (subtractionResult * 0.4)).toFixed(2);
            finalCashout = Math.max(adjustedSubtraction, 1.01);
        }

        // Check if finalCashout is lower than 1.30
        if (finalCashout < 1.30) {
            finalCashout = 0; // Fill in the number 0 in the box
        } else if (finalCashout < 1) {
...
function getHashObject(md5) {
    ...
    var hashValues = [
	"265806819fd9246a2fbb280e97f0c217dc59e4d4da195a2663a7754990477ce1",
	"0d5a507b517d98c1136599eb9d8f0283d014b46fe52311fd2f13292e15e9d3b5",
	"cf60e8940cb0e73f81ea2d64e8ddbca4b5a577f2f874091e66e4960298cae1b7",
	"387e4b259ee816b1242215437a7c171a5d87ad659487f7e37ef16003ed62d8be",
	"91594126d2c14026e56e7581fa9398fe32a78fa2b92c06f21811c47179d192e4",
	"4bab7f769262d0fba81d4a6df8267caab17982180bf87a1bad331b00287eac1d",
    ...
    var hashValueSum = hashValues.reduce((sum, value) => sum + (2.00 + 0.99 * hashValueNumebr / (100 * getHashValueNumber(value, 13) - hashValueNumebr)), 0);
